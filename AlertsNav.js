...
class AlertsNav {
  ...
  init() {
    // Bind events here
    this.$scroller.on('scroll', this.update.bind(this, false));
  ...
  scrollTo(e) {
    ...
    let $to = $('[data-slide]').eq((String($(e.currentTarget).data('nav')).match(/slide\-([0-9]+)/))[1]|0);

    if(typeof TweenMax !== 'undefined') {
      TweenMax.to(window, 100 / this.scrollDuration, {scrollTo: {y: $to, offsetY: this.scrollOffsetY + this.scrollOffsetMenu, autoKill:false}});
    }else{
